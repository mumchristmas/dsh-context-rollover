...
**倒数就在你本来就会看的地方。** 会话统计行里的图标*就是*模式 —— 循环箭头代表本会话滚动归档，
方块拆分代表保留自己的压缩后端 —— 悬停它会告诉你下一步是什么、还剩多少 Token 增长空间：
`20K Token后提示`、`已提示 · 60K Token后预警`、`自动滚动就位 · 20K Token后强制换窗`，
如果是会被摘要的会话则是 `300K Token后强制压缩`。气泡用的是应用自带的 tooltip 样式与配色。

<img src="assets/context-mode-bubble.webp" alt="滚动归档控件与它的悬停提示" width="620">

点击不再直接切换模式，而是展开一个面板，因为读者第一眼要的是数字：本窗口相对三个点的位置，
以及唯一能改策略的那个开关。进度条读起来像个信号灯——色带和刻度按触发顺序是绿、黄、红，
一眼就能看出窗口正处在哪一段、下一个点是哪一个；下方的图例写明每个刻度叫什么、落在百分之几。
改了阈值进度条就跟着变：它画的是**生效**配置，所以阶梯变短时是少画一个刻度，而不是两个刻度叠在同一像素上。
...
表单在每个宿主上只有一个落点。DSH 0.1.6-alpha.2 及之后：打开侧边栏的 **Plugins**，点开
`context-rollover` 这个 bundle，配置就在它页面的最上方、排在它的行之前。更早的宿主上，它是
**设置 → 插件配置** 里的那张卡片。两处注册同时挂上，各自等自己宿主的声明，所以升级只会把
表单换个位置，不会动任何一个设置值。
